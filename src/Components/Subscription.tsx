...
import {shouldShowTrialCard, isTrialActive} from '../utils/trialUtils';
...
const Subscription: React.FC<SubscriptionProps> = ({
  ...
  const handleToggleSubscriptionInfo = () => {
    setShowSubscriptionInfo(prev => {
      ...
      // Track subscription info view
      if (next && onSubscriptionInfoViewed) {
        onSubscriptionInfoViewed();
      }
      return next;
    });
  ...
  // Log when trial card should be rendered
  useEffect(() => {
    const shouldShow = shouldShowTrialCard(
      preferences.isIOSActive,
      preferences.isInTrial,
      preferences.trialInstallationDate,
    );
  ...
  const handleTrialSelection = async () => {
    try {
      ...
      // Track trial selection
      if (onTrialSelected) {
        onTrialSelected();
      }

      // Check if user already had a trial (re-installation case)
      const existingDate = preferences.trialInstallationDate;
      const existingTrialStatus = preferences.isInTrial;
      
      // If user already had a trial and it's expired, don't allow new trial
      if (
        existingDate &&
        existingDate !== 'init.NotSet' &&
        !isTrialActive(existingDate)
      ) {
        showError(
      ...
      // If user is already in an active trial, just grant access
      if (existingTrialStatus === '1' && existingDate && existingDate !== 'init.NotSet' && isTrialActive(existingDate)) {
        setGotSubscription(true);
        setPurchaseSuccess(true);
        setIsLoading(false);
        return;
      ...
      // Set trial status and installation date
      await setItem('isInTrial', '1');
      
      // Set installation date to current timestamp if not already set or if previous trial expired
      const currentTimestamp = Date.now().toString();
      if (!existingDate || existingDate === 'init.NotSet' || !isTrialActive(existingDate)) {
    ...
    <View style={styles.container}>
      {isLoadingSubscriptions ? (
        ...
        <ScrollView
          ...
          {/* Toggle Switch for Yearly/Monthly when trial card is shown */}
          {shouldShowTrialCard(
            preferences.isIOSActive,
            preferences.isInTrial,
            preferences.trialInstallationDate,
          ...
          <View style={dynamicStyles.cardsContainer}>
            ...
            {/* Show yearly card only when showYearlyPlan is true or when no trial card is shown */}
            {(showYearlyPlan ||
              !shouldShowTrialCard(
                preferences.isIOSActive,
                preferences.isInTrial,
            ...
            {/* Show monthly card only when showYearlyPlan is false or when no trial card is shown */}
            {(!showYearlyPlan ||
              !shouldShowTrialCard(
                preferences.isIOSActive,
                preferences.isInTrial,
