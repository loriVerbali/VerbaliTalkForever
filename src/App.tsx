...
const App = () => {
  ...
  const runBootstrap = async () => {
    try {
      const result = await sessionManager.bootstrapDevice();
      if (result.success) {
        setBootstrapStatus('ok');
      } else if (result.error === 'device_revoked' || result.error === 'organization_inactive') {
        setBootstrapStatus('revoked');
        // Trigger TTS
        TTSService.speak('There seems to be an issue please ask an adult for help');
      } else {
        // Network errors shouldn't block use if we are already enrolled
        setBootstrapStatus('ok');
      }
    } catch (error) {
      setBootstrapStatus('ok');
